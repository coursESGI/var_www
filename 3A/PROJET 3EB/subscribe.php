...
<?php
    ...
    if( isset($_POST['nom']) &&  isset($_POST['prenom']) &&  isset($_POST['dateDeNaissance']) &&  isset($_POST['ville']) && 
     isset($_POST['email']) &&  isset($_POST['motDePasse']) &&  isset($_POST['confirmationMotDePasse']) &&    isset($_POST['genre']) ){
        ...
        /*try{
            $bdd = new PDO(BDD_DRIVER.":host=".BDD_HOST.";dbname=".BDD_DBNAME , BDD_USER, BDD_PWD);    
        }catch(Exception $e){
            die("Error :".$e->getMessage());
        }*/

        $bdd = connectBdd();
		$user = getUser($bdd, ["email"=>$_POST["email"]], "id");
      ...
      if($error){
		...
		setUser($bdd, ["name"=>$_POST['nom'],
            ...
            /*
                ...
                INSERT INTO users (name,surname,kind,email,pwd,comment,country,birthday)
                    VALUES ("skrzypczyk", "yves", ...);      
                UPDATE users SET name="toto" WHERE id=3;
